...
void* lista_elemento_en_posicion(lista_t* lista, int posicion){
	if(!lista || posicion >= lista->cantidad || posicion < 0)
		return NULL;
...
nodo_t* ptr_nodo_en_pos (lista_t* lista, int posicion){
	if(posicion < 0)
		return NULL;	
	nodo_t* nodo_actual = lista->primer_nodo;
	size_t i = 0;
...
int lista_insertar_en_posicion(lista_t* lista, void* elemento, int posicion){
	if(!lista || posicion < 0)
		return ERROR;
...
int lista_borrar_de_posicion(lista_t* lista, int posicion){
	if(lista_vacia(lista) || posicion < 0)
		return ERROR;
	nodo_t* aux;
	if(posicion >= lista->cantidad)
		posicion = (int)lista->cantidad - 1 ;
	if(posicion == 0){
...
int lista_borrar(lista_t* lista){
	if(!lista)
		return ERROR;
	return lista_borrar_de_posicion(lista, (int)lista->cantidad -1);
}
